...
namespace SchoolChoicePlayground.Tests.Models
{
    ...
    public class AppRepoUserTests
    {
        ...
        private void ConnectMocksToDataStore(IEnumerable<SchoolAppUser> data_store)
        {
            var data_source = data_store.AsQueryable<SchoolAppUser>();
            mock_user_set.As<IQueryable<SchoolAppUser>>().Setup(data => data.Provider).Returns(data_source.Provider);
            mock_user_set.As<IQueryable<SchoolAppUser>>().Setup(data => data.Expression).Returns(data_source.Expression);
            mock_user_set.As<IQueryable<SchoolAppUser>>().Setup(data => data.ElementType).Returns(data_source.ElementType);
            mock_user_set.As<IQueryable<SchoolAppUser>>().Setup(data => data.GetEnumerator()).Returns(data_source.GetEnumerator());

            mock_context.Setup(a => a.SchoolAppUsers).Returns(mock_user_set.Object);
        }
        ...
        private void AddMockUsersAndSchoolsToDb()
        {
            ...
            var users = new List<SchoolAppUser>
            {
                new SchoolAppUser {UserId = 123, name = "Tom Griffey" },
                new SchoolAppUser {UserId = 456, name = "Soccer Mom"}
            };
        ...
        public void Initialize()
        {
            ...
            mock_user_set = new Mock<DbSet<SchoolAppUser>>();
        ...
        {
            SchoolAppContext context = mock_context.Object;
        ...
        {
            AddMockUsersAndSchoolsToDb();
            var newUser = new SchoolAppUser { UserId = 777, name = "Deliquent Parent" };
        ...
        {
            var users = new List<SchoolAppUser>();
            SchoolAppUser first_user = new SchoolAppUser {
                UserId = 999,
                name = "Tom",
                email = "tgriffey@charter.net"
            };

            SchoolAppUser second_user = new SchoolAppUser
            {
                UserId = 434,
                name = "Joe",
                email = "tgriffey@charter.net"
        ...
        {
            // Arrange
            var expected = new List<SchoolAppUser>
            {
                new SchoolAppUser {UserId = 123, name = "Tom Griffey" },
                 new SchoolAppUser {UserId = 456, name = "Soccer Mom" }
            };
